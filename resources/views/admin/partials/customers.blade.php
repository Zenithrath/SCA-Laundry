<!-- TABEL DATA PESANAN -->
<div class="space-y-6 animate-fade-in relative px-2 sm:px-0">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                Kelola status pesanan dan lihat detail.
            </p>
        </div>
    </div>

    <!-- Tabel Pesanan -->
    <div class="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-[30px] shadow-lg border border-slate-100 dark:border-slate-700">
        <div class="overflow-x-auto rounded-2xl sm:rounded-[30px]">
            <table class="min-w-[700px] w-full text-left text-xs sm:text-sm text-slate-600 dark:text-slate-300">
                <thead class="bg-blue-50 dark:bg-slate-700 border-b border-blue-100 dark:border-slate-600 text-slate-700 dark:text-slate-200">
                    <tr>
                        <th class="p-3 sm:p-4 whitespace-nowrap">Order ID</th>
                        <th class="p-3 sm:p-4 whitespace-nowrap">Pelanggan</th>
                        <th class="p-3 sm:p-4 whitespace-nowrap">Layanan</th>
                        <th class="p-3 sm:p-4 whitespace-nowrap">Status</th>
                        <th class="p-3 sm:p-4 text-right whitespace-nowrap">Aksi</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors">
                        <td class="p-3 sm:p-4 font-bold text-slate-700 dark:text-white whitespace-nowrap">
                        ...
                        <td class="p-3 sm:p-4 whitespace-nowrap">
                            ...
                            <div class="text-xs sm:text-sm font-bold text-green-600 mt-1">
                        ...
                        <td class="p-3 sm:p-4 whitespace-nowrap">
                            ...
                            <span id="badge-cust-{{ $order->id }}"
                                class="px-3 py-1 rounded-full text-[9px] sm:text-[10px] font-bold border {{ $statusClass }}">
                                {{ $order->status }}
                            </span>
                        </td>
                        ...
                        <td class="p-3 sm:p-4 text-right whitespace-nowrap">
                            <button 
                                ...
                                class="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-500/30">
                                Detail
                            </button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="p-6 sm:p-8 text-center text-slate-500 dark:text-slate-400 italic">
...
<div id="customerModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/60 backdrop-blur-sm p-2 sm:p-4 transition-opacity opacity-0">
    <div id="customerModalContent"
        class="w-full max-w-2xl bg-white dark:bg-slate-900 text-slate-800 dark:text-white shadow-2xl rounded-2xl sm:rounded-[30px] p-4 sm:p-8 relative transform scale-95 transition-transform duration-300 overflow-y-auto max-h-[90vh]">
        
        <button onclick="window.closeCustomerModal()"
            class="absolute top-4 sm:top-6 right-4 sm:right-6 text-2xl opacity-50 hover:opacity-100 transition z-50 cursor-pointer">
            &times;
        </button>

        <h3 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 border-b pb-3 sm:pb-4 dark:border-slate-700">
            Detail Pesanan
        </h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
            ...
            <div class="space-y-4 sm:space-y-5 text-sm">
                <div>
                    ...
                    <p id="cModalCode" class="font-mono font-bold text-base sm:text-lg bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded w-fit mt-1"></p>
                </div>

                <div>
                    <label class="text-xs font-bold uppercase text-slate-400">Pelanggan</label>
                    <p id="cModalName" class="font-bold text-base sm:text-lg"></p>
                    <p id="cModalPhone" class="text-slate-500 text-sm"></p>
                </div>

                <div>
                    <label class="text-xs font-bold uppercase text-slate-400">Alamat Jemput</label>
                    <div id="cModalAddress"
                        class="bg-slate-50 dark:bg-slate-800 p-3 rounded-xl text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed border border-slate-100 dark:border-slate-700 mt-1"></div>
                </div>

                <a id="cBtnWA" href="#" target="_blank"
                    class="flex items-center justify-center gap-2 w-full py-3 bg-green-500 text-white rounded-xl font-bold hover:bg-green-600 transition shadow-lg shadow-green-500/30 text-sm">
                    Chat WhatsApp
                </a>
            </div>
            ...
            <div class="space-y-5 sm:space-y-6">
                ...
                <div class="p-4 sm:p-5 rounded-2xl bg-blue-50 dark:bg-slate-800 border border-blue-100 dark:border-slate-700 text-sm">
                    ...
                    <div class="flex justify-between font-bold text-base sm:text-lg text-slate-800 dark:text-white">
                        <span>Total</span>
                ...
                <div>
                    <label class="text-xs font-bold uppercase tracking-wider block mb-3 text-slate-400">
                        Update Status
                    </label>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2" id="cStatusContainer"></div>
                </div>
            </div>
        </div>
    </div>
